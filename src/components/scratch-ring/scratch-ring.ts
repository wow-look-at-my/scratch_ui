...
class ScratchRing extends HTMLElement {
  ...
  private _rect: ScratchRingRect | null = null;
  ...
  static burst(target: Element | null, opts: ScratchRingOpts = {}) {
    if (!target) return;
    const rect = target.getBoundingClientRect();
    if (!rect.width || !rect.height) return;
    const ring = document.createElement('scratch-ring') as ScratchRing;
...
declare global {
  interface Window {
    /* Optional on purpose: a page that never loads this module has no ring,
       and every caller degrades to no burst rather than throwing. */
    ScratchRing?: typeof ScratchRing;
  }
  interface HTMLElementTagNameMap {
    'scratch-ring': ScratchRing;
  }
}
